...
class Sed(Conditioner):
    ...
    def __init__(self, _oWatcher: "Watcher", _sConfiguration):  # noqa: D107
        ...
        # ... logical not
        self.__bNot = False
        if "not" in dConfiguration:
            self.__bNot = True

        # ... raw data
        self.__bRaw = False
        if "raw" in dConfiguration:
            self.__bRaw = True

        # ... replace / group
        self.__sReplace = None
        self.__bExpand = False
        if "replace" in dConfiguration:
            self.__sReplace = dConfiguration["replace"][0]
            if not self.__bNot and re.match(r"(^|[^\\])\\[0-9]", self.__sReplace):
                self.__bExpand = True
    ...
    def feed(self, _oData: "Data") -> "Data | None":  # noqa: D102
        ...
        if self.__sReplace is not None:
            return Data(
                _oData.watcher, _oData.data_raw, oMatch.expand(self.__sReplace) if self.__bExpand else self.__sReplace
            )
        else:
            return _oData
