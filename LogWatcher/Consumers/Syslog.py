...
class Syslog(Consumer):
    ...
    def __init__(self, _oWatcher: "Watcher", _sConfiguration):  # noqa: D107
        ...
        if "facility" in dConfiguration:
            ...
            if sFacility in dFacilities:
                self.__iFacility = dFacilities[sFacility]
            else:
                _oWatcher.log(
                    "ERROR[Consumer:Syslog(%s)]: Invalid 'facility' configuration parameter\n" % _oWatcher.name()
                )
                raise ValueError(
                    "Syslog facility must be among AUTH, CRON, DAEMON, KERN, LOCAL{0..7}, LPR, MAIL, NEWS, SYSLOG, USER or UUCP"
                )

        # ... level
        self.__iLevel = syslog.LOG_INFO
        if "level" in dConfiguration:
            dLevels = {
            ...
            if sLevel in dLevels:
                self.__iLevel = dLevels[sLevel]
            else:
                _oWatcher.log(
                    "ERROR[Consumer:Syslog(%s)]: Invalid 'level' configuration parameter\n" % _oWatcher.name()
                )
        ...
        self.__sFormatter = "<%d>LogWatcher/%s[%d]: %%s" % (
            self.__iFacility + self.__iLevel,
            _oWatcher.name(),
            _oWatcher.pid(),
        )
