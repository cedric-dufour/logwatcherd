...
class Daemon:
    ...
    def __initArgumentParser(self):
        ...
        # ... debug
        self.__oArgumentParser.add_argument(
            "-d", "--debug", action="store_true", default=False, help="Enable debugging messages"
        )
    ...
    def __initConfigObj(self):
        ...
        try:
            self.__oConfigObj = configobj.ConfigObj(
                self.__oArguments.config, configspec=LOGWATCHER_CONFIGSPEC, file_error=True
            )
        except Exception as e:
            self.__oConfigObj = None
        ...
        bDebug = False
        lsIncludeGlobs = []
        if "LogWatcher" in self.__oConfigObj:
            with contextlib.suppress(ValueError):
                bDebug = self.__oConfigObj["LogWatcher"].get("debug", "0").lower() in ["1", "t", "true", "y", "yes"]
            lsIncludeGlobs = self.__oConfigObj["LogWatcher"].get("includes", [])

        # ... includes
        for sIncludeGlob in lsIncludeGlobs:
            if not len(sIncludeGlob):
                continue
            if sIncludeGlob[0] != "/":
            ...
            for sIncludeFile in sorted(glob.glob(sIncludeGlob)):
                try:
                    oConfigObj = configobj.ConfigObj(sIncludeFile, configspec=LOGWATCHER_CONFIGSPEC, file_error=True)
                    ...
                    self.__oConfigObj = None
                    sys.stderr.write(
                        "ERROR[Daemon]: Failed to include configuration from '%s'; %s\n" % (sIncludeFile, str(e))
                    )
                    return errno.ENOENT
        ...
        if oValidatorResult is not True:
    ...
    def __spawnWatchers(self, _oConfigObj):
        ...
        if _oConfigObj["LogWatcher"]["delay"] > 0:
            if self.__bDebug:
                sys.stderr.write(
                    "DEBUG[Daemon]: Waiting %d seconds before start...\n" % _oConfigObj["LogWatcher"]["delay"]
                )
        ...
        for sWatcherName in _oConfigObj.keys():
            ...
            try:
                oPluginClass = getattr(
                    __import__("LogWatcher.Producers.%s" % sPluginName, fromlist=["LogWatcher.Producers"]), sPluginName
                )
            ...
            try:
                for sPluginConfig in dWatcherConfig["filters"]:
                    ...
                    try:
                        oPluginClass = getattr(
                            __import__("LogWatcher.Filters.%s" % sPluginName, fromlist=["LogWatcher.Filters"]),
                            sPluginName,
                        )
                        ...
                        if self.__bDebug:
                            sys.stderr.write(
                                "DEBUG[Daemon(%s)]: Filter instantiated (%s)\n" % (sWatcherName, sPluginName)
                            )
                    except Exception as e:
                        sys.stderr.write(
                            "ERROR[Daemon(%s)]: Invalid filter (%s)\n%s\n" % (sWatcherName, sPluginName, str(e))
                        )
                        if self.__bDebug:
                            traceback.print_exc()
                        raise
            ...
            try:
                for sPluginConfig in dWatcherConfig["conditioners"]:
                    ...
                    try:
                        oPluginClass = getattr(
                            __import__(
                                "LogWatcher.Conditioners.%s" % sPluginName, fromlist=["LogWatcher.Conditioners"]
                            ),
                            sPluginName,
                        )
                        ...
                        if self.__bDebug:
                            sys.stderr.write(
                                "DEBUG[Daemon(%s)]: Conditioner instantiated (%s)\n" % (sWatcherName, sPluginName)
                            )
                    except Exception as e:
                        sys.stderr.write(
                            "ERROR[Daemon(%s)]: Invalid conditioner (%s)\n%s\n" % (sWatcherName, sPluginName, str(e))
                        )
                        if self.__bDebug:
                            traceback.print_exc()
                        raise
            ...
            try:
                for sPluginConfig in dWatcherConfig["consumers"]:
                    ...
                    try:
                        oPluginClass = getattr(
                            __import__("LogWatcher.Consumers.%s" % sPluginName, fromlist=["LogWatcher.Consumers"]),
                            sPluginName,
                        )
                        ...
                        if self.__bDebug:
                            sys.stderr.write(
                                "DEBUG[Daemon(%s)]: Consumer instantiated (%s)\n" % (sWatcherName, sPluginName)
                            )
                    except Exception as e:
                        sys.stderr.write(
                            "ERROR[Daemon(%s)]: Invalid consumer (%s)\n%s\n" % (sWatcherName, sPluginName, str(e))
                        )
                        if self.__bDebug:
                            traceback.print_exc()
                        raise
        ...
        while True:
            ...
            if self.__bStop:
                ...
                while True:
                    ...
                    if iThreadsActive > 1:
                        if self.__bDebug:
                            sys.stderr.write(
                                "DEBUG[Daemon]: Stopping / Waiting for %d threads to terminate\n" % (iThreadsActive - 1)
                            )
                        time.sleep(1)
                    else:
                        break
            ...
            if iThreadsActive > 1:
                ...
                if iThreadsWatchers != iWatchers:
                    sys.stderr.write(
                        "WARNING[Daemon]: Only %d/%d watchers are running\n" % (iThreadsWatchers, iWatchers)
                    )
                if self.__bDebug:
    ...
    def run(self):
        ...
        if self.__oArguments.plugin_help is not None:
            ...
            sPluginType = (
                self.__oArguments.plugin_help[0:iSeparator]
                .capitalize()
            ...
            sPluginName = self.__oArguments.plugin_help[iSeparator + 1 :].capitalize()
            try:
                oPluginClass = getattr(
                    __import__(
                        "LogWatcher.%ss.%s" % (sPluginType, sPluginName), fromlist=["LogWatcher.%ss" % sPluginType]
                    ),
                    sPluginName,
                )
                help(oPluginClass)
